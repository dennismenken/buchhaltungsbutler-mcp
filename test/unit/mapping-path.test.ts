...
// Die sechs Pfadregeln aus dem Kopf von src/mapping/path.ts, jede einzeln, plus die beiden
// Negativprüfungen: Der Body trägt kein `id_by_customer` (Pfadregel 5), und der
// zurückgegebene `specPath` ist unverändert der Spezifikationspfad (Pfadregel 6).

function buildTemplatePath(entry: ToolEntry, args: Record<string, unknown>) {
  return buildPath(entry, args);
}

describe("Pfadregel 1 und 2: Wertprüfung und Kodierung", () => {
  ...
  it("hält das Muster aus Pfadregel 1 fest", () => {
  ...
  it("kodiert den eingesetzten Wert, auch wenn Pfadregel 1 das bereits erledigt", () => {
    // Nachweis über die Vorlage eines hypothetischen Eintrags: Der Pfad enthält danach kein
    // rohes Sonderzeichen mehr. Die Prüfung aus Pfadregel 1 greift vorher; dieser Test belegt,
...
describe("Pfadregel 3: Rückprüfung gegen die Vorlage", () => {
...
describe("Pfadregel 4: die übrigen 50 Endpunkte", () => {
...
describe("Pfadregel 5, Negativprüfung: der Body trägt kein id_by_customer", () => {
...
describe("Pfadregel 6, Negativprüfung: specPath bleibt der Spezifikationspfad", () => {
...
describe("Die vier betroffenen Endpunkte, vollständig", () => {
  it("deckt genau die vier Pfade ab", () => {
    ...
    // fehlt hier bewusst, weil sie nichts Neues prüfte.
  });

  it("kennzeichnet die schreibende Form als nicht verifiziert", () => {
