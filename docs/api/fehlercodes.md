...
- Antwortumschlag bei Fehler, eigene lesende Messung am 2026-09-13 (Protokoll mit Aufrufform, HTTP-Status und wörtlichem Antwortkörper: `docs/api/grundlagen.md`, Abschnitt 2.5, Aufrufe M2 und M3): `{"success":false,"error_code":<int>,"message":"<text>"}`. Die einzelnen `_ErrorCode*`-Definitionen in der Spec bilden genau die Felder `error_code` und `message` dieses Umschlags ab.
...
> Die Zahlen oben sind richtig, aber sie beschreiben die **Definitionen**, nicht den Katalog.
> Der Fehlerkatalog der Umsetzung wird nach dem Paar
