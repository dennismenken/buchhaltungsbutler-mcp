...
// (Plan 2 Dateibaum, 7.5).
//
// Die Tabelle steht hier und nicht in den sieben Registereinträgen, weil drei Stellen
// dieselbe Zahl brauchen: das Schema (`maximum`, `default`), die Querprüfung Q2 (Plan 4.7)
// und die Bestandszeile der Antwort (7.5). Eine zweite Liste wäre eine zweite Wahrheit.
//
// **Es gibt keine Gesamttrefferzahl.** `rows` ist die Zeilenzahl DIESER Antwort (Plan 0.3
// L5). Die Abbruchbedingung beim Blättern ist „weniger Zeilen als limit“, nicht `rows`.
 ...
 * Request abgeht, und zwar unsichtbar (Plan 7.5, R4). Ein fehlendes Maximum kostet im
...
export interface PaginationSpec {
  ...
  /** Unsere Vorgabe. Sie wird immer gesendet, auch wenn der Aufrufer sie weglässt (7.5). */
 ...
 * (Plan 7.5). Geschlüsselt ist nach dem **Spezifikationspfad**, nicht nach dem gebauten
 * Pfad: Bei den vier Endpunkten mit Platzhalter wäre der gebaute Pfad nicht konstant
 * (Plan 4.6) — sie führen allerdings ohnehin keine Paginierung.
 *
 * Die drei `settings`-Listen tragen bewusst kein Maximum; die Begründung steht oben bei
 * {@link PaginationSpec} und in Plan 7.5. Sobald ein lesender Aufruf eine dieser Grenzen
 * klärt, wird sie hier nachgetragen **und** das betroffene Werkzeug in Q2 aufgenommen
 * (Plan 4.7); beides gehört zusammen.
 */
...
/**
 * Der Warnsatz für die drei Listen ohne dokumentierte Obergrenze, wörtlich nach Plan 7.5.
 ...
 * deshalb sendet der Server `limit` immer mit (Plan 7.5 Regel 1); der Satz sagt dem
...
/**
 * `limit` mit endpunktspezifischem Maximum und konservativer Vorgabe (Plan 4.5).
 *
 * Die Vorgabe steht im Schema und nicht im Request-Mapper: Zod setzt sie beim Parsen ein,
 * damit trägt jeder geprüfte Aufruf ein `limit`, und Regel 1 aus 7.5 ist konstruktiv
 * erfüllt statt an einer zweiten Stelle nachgebaut.
 *
...
/** `offset` nach Plan 4.5. Die API kennt keinen Cursor und keine Gesamttrefferzahl. */
 ...
 * (Plan 4.7) und damit der Grund, warum Q2 an den drei `settings`-Listen ausdrücklich
 * **nicht** hängt.
 */
 ...
 *   bleibt der Kontenrahmen dann jenseits der ersten Seite unerreichbar (Plan 4.1).
 */
