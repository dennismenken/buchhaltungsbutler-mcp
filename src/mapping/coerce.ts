// Typnormalisierung der Antwort, deterministisch und an genau einer Stelle (Plan 7.4).
//
// Die Tabelle aus 7.4, vollständig und als Code:
//
...
//
// Drei Regeln sind dabei nicht verhandelbar:
//
//  1. **Es entsteht niemals ein `number`-Betragsfeld.** Die Rohzeichenkette bleibt führend,
//     das Centfeld ist ihre verlustfreie Ganzzahlform (Streitfrage S9).
//  2. **Kennungen gehen immer als String hinaus** (Streitfrage S10). Gemessen liefert die API
//     sie bei `receipts` als String und bei `transactions` als Zahl (Befund L3); eine
...
//     kommen nicht gesetzte Felder durchgehend als JSON-`null` (Befund L3); eine Warnung je
//     leerem Feld wäre an einer Belegliste Lärm und würde die echten Abweichungen verdecken.
...
/** Die Endung des zusätzlichen Ganzzahlfeldes eines Betrages (Plan 7.4). */
 ...
 *
 * `"0"` und `"1"` sind die gemessene Form (Befund L3). `"true"` und `"false"` werden
...
export interface CoercedField {
  ...
  /** Zusätzliches Feld, derzeit ausschließlich `<feld>_cents` (Plan 7.4). */
  ...
  switch (type) {
    case "amount-string": {
      if (typeof value !== "string") {
        ...
        // genau der Fehler, den S9 ausschließt. Der Wert geht unverändert hinaus, mit Meldung.
    ...
    case "id-string": {
      ...
      if (typeof value === "number" && Number.isInteger(value)) {
        ...
        // und zwar zum String hin (S10). Das ist keine Umdeutung, sondern dieselbe Kennung.
 ...
 * (Plan 7.3, letzter Fall; Befund L4: `amount_paid` und `amount_paid_fixed` kommen, obwohl
 * die Spezifikation sie nicht kennt). Verwerfen wäre Datenverlust, und ein geschlossener
 ...
 * `parseFloat` des Rohstrings (Plan 7.4).
 */
